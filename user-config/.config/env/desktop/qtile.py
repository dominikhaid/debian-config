...
keys = [
        ...
        Key([mod], "p", lazy.spawn(".config/rofi/bin/tizonia-run.sh")),
...
layout_theme = {
        "border_width": 3,
        "margin": 5,
        "border_focus": notify_bg,
        "border_normal": sys_bg,
        }
...
widget_defaults = dict(font="FiraMono",margin=0, borderwidth=0, fontsize=16, linewidth=0, padding=15, background=panel_color)
...
def init_screens():
    return [
            Screen(top=bar.Bar(widgets=init_widgets_screen1(), opacity=1, size=40)),
            Screen(top=bar.Bar(widgets=init_widgets_screen2(), opacity=1, size=40)),
            Screen(top=bar.Bar(widgets=init_widgets_screen3(), opacity=1, size=40)),
            ]
